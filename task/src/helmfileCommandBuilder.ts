...
export class helmfileCommandBuilder {
  ...
  private addArguments() {
    ...
    if (tl.getInput(inputNames.selector)) {
      const selectors = tl.getDelimitedInput(inputNames.selector, '\n');
      if (selectors) {
        this._helmfile.addArgument(
          selectors
            .map(selector => `-l ${selector.trim()}`)
            .join(' ')
            .trim()
        );
      }
    ...
    if (tl.getInput(inputNames.commandArgs)) {
      const commandArgs = tl.getDelimitedInput(inputNames.commandArgs, '\n');
      if (commandArgs) {
        this._helmfile.addArgument(
          commandArgs
            .map(commandArg => `${commandArg.trim()}`)
            .join(' ')
            .trim()
        );
      }
